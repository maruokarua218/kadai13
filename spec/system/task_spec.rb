...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe '一覧表示機能' do
    ...
    context '終了期限でソートするボタンを押した場合' do
      it '終了期限が遅いタスクが一番上に表示される' do
       FactoryBot.create(:task, title: 'first', deadline: Date.today, user: user)
       FactoryBot.create(:task, title: 'second', deadline: Date.today + 1.days, user: user)
       visit new_session_path
       fill_in "session_email", with: 'qq@gmail.com'
       fill_in 'session_password', with: 'qqdayo'
       click_on 'Log in'
    ...
    context '優先順位でソートするボタンを押した場合' do
      it '優先順位が高いタスクが一番上に表示される' do
       FactoryBot.create(:task, title: 'first', priority: '低', user: user)
       FactoryBot.create(:task, title: 'second', priority: '高', user: user)
       visit new_session_path
       fill_in "session_email", with: 'qq@gmail.com'
       fill_in 'session_password', with: 'qqdayo'
       click_on 'Log in'
  ...
  describe '詳細表示機能' do
     context '任意のタスク詳細画面に遷移した場合' do
       it '該当タスクの内容が表示される' do
         @task = FactoryBot.create(:task, title: 'aa', content: 'aa', user: user)
         visit new_session_path
         fill_in "session_email", with: 'qq@gmail.com'
         fill_in 'session_password', with: 'qqdayo'
         click_on 'Log in'
  ...
  context 'ステータス検索をした場合' do
    it "ステータスに完全一致するタスクが絞り込まれる" do
      @task = FactoryBot.create(:task, status: '完了', user: user)
      visit new_session_path
      fill_in "session_email", with: 'qq@gmail.com'
      fill_in 'session_password', with: 'qqdayo'
      click_on 'Log in'
  ...
  context 'タイトルのあいまい検索とステータス検索をした場合' do
    it "検索キーワードをタイトルに含み、かつステータスに完全一致するタスク絞り込まれる" do
      @task = FactoryBot.create(:task, title: 'aa', content: 'aa', status: '完了', user: user)
      visit new_session_path
      fill_in "session_email", with: 'qq@gmail.com'
      fill_in 'session_password', with: 'qqdayo'
      click_on 'Log in'
